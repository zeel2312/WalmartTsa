...
# Add dropdown menu for selecting statistics in checkbox 5
if st.sidebar.checkbox("Checkbox 5", key='select_to_view_predictions'):
    selected_stat_5 = st.selectbox("Select to view Predictions", ["LSTM Model Predictions"])

# # Show selected statistics if checkbox 5 is selected
# if st.sidebar.checkbox("Show Checkbox 5"):
#     if selected_stat_5 == "LSTM Model Predictions":
...
#         df_pred = df_pred.rename(columns={2:"Weekly_Sales"})
        ...
        # # Assuming y_pres_train_sum, y_pres_test_sum, and df_pred_sum are your arrays or Series containing the data
        # # Create DataFrame for Plotly Express
        # plot_data = pd.DataFrame({
        #     'training': y_pres_train_sum,
        #     'actual': y_pres_test_sum,
        #     'forecast': df_pred_sum,
        #     'index': range(len(df_pred_sum))
        # })
    ...
    if selected_stat_5 == "LSTM Model Predictions":
        ...
        # Assuming you have already prepared your dataset `df` with the necessary columns
        x = df[['Temperature', 'Fuel_Price', 'CPI', 'Unemployment', 'Holiday_Flag']].values
        y = df['Weekly_Sales'].values

        # Normalize your input features if required
        ...
        # If scaling was used, invert predictions back to original scale
        # y_pred = scaler.inverse_transform(y_pred)

        # Plotting the results
        test_dates = df['Date'].iloc[test_index]
        fig = px.line(x=test_dates, y=[y_test.flatten(), y_pred.flatten()],
                    labels={'value': 'Weekly Sales', 'variable': 'Type'},
                    title='Actual vs Forecasted Weekly Sales')
        fig.data[0].name = 'Actual'
        fig.data[1].name = 'Forecast'
        fig.update_layout(xaxis_title='Date', yaxis_title='Weekly Sales', legend_title='Type')
        st.plotly_chart(fig)
